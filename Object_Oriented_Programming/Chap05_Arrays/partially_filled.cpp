...
void fillArray(int a[], int size, int& numberUsed);
    ...
    // a[0] through a[numberUsed - 1] have been filled with nonnegative integers read form the keyboard
...
void showDifference(const int a[], int numberUsed);
    ...
    // Postcondition: Gives screen output showing how much each of the first numberUsed elements of the array a differs from their average
...
// 이 프로그램에서 numberUsed는 배열의 "실제 사용된 크기"를 추적하는 변수임.
// C++는 배열이 얼마나 채워져 있는 것인지 스스로 알 수 없으므로 따로 관리하는 것